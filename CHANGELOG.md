...
- **Rank Math Product schema missing aggregateRating**: Google Rich Results Test showed "Falta el campo aggregateRating (opcional)" warning on the Product node generated by Rank Math. Now hooks into `rank_math/json_ld` filter to inject `AggregateRating` (4.7 / 395 reviews) directly into Rank Math's `@graph` Product entity.
- **Duplicate Product structured data**: Disabled WooCommerce's default `WC_Structured_Data` output on tour pages (`ovatb_tour` product type) to prevent a third competing Product schema.
