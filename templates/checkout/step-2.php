...
<form id="wiwa-checkout-step-2" class="wiwa-checkout-form checkout woocommerce-checkout" method="post" enctype="multipart/form-data">
    <h2><?php _e('Selecciona tu medio de pago', 'wiwa-checkout'); ?></h2>
    
    <div class="currency-selector-section">
        <h3><?php _e('Moneda de pago', 'wiwa-checkout'); ?></h3>
...
endforeach; ?>
            ...
            <span class="currency-loading" style="display: none;">
                ...
                <?php _e('Actualizando...', 'wiwa-checkout'); ?>
            </span>
        </div>
        <p class="description"><?php _e('El precio se actualizará automáticamente según la moneda seleccionada.', 'wiwa-checkout'); ?></p>
    </div>
    
    <hr class="section-divider">
    
    <div class="payment-methods-section">
        <h3><?php _e('Método de pago', 'wiwa-checkout'); ?></h3>
...
?>
    ...
    endforeach;
                ...
                <div class="woocommerce-info"><?php _e('No hay métodos de pago disponibles.', 'wiwa-checkout'); ?></div>
            <?php
endif; ?>
        </div>
    </div>
        ...
        if (typeof woocs_redirect !== 'undefined') {
            ...
            $.ajax({
                ...
                error: function() {
                    ...
                    alert('<?php _e('Error al cambiar la moneda. Por favor recarga la página.', 'wiwa-checkout'); ?>');
                }
            });
        } else {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        alert(response.data.message || '<?php _e('Error al cambiar la moneda', 'wiwa-checkout'); ?>');
                        $loading.hide();
                        $currencySelect.prop('disabled', false);
                    }
                },
                error: function() {
                    $loading.hide();
                    $currencySelect.prop('disabled', false);
                    alert('<?php _e('Error en la solicitud', 'wiwa-checkout'); ?>');
                }
            });
        }
    });
